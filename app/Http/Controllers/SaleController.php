...
class SaleController extends Controller
{
    ...
    public function getProduct($id)
    {
        ...
        ->select('product_warehouse.*',  'products.is_embeded','product_variants.id')
        ->get();
        ...
        foreach ($lims_product_warehouse_data as $product_warehouse) 
        {
            ...
            $expired_date[] = null;
            $variantIds [] = $product_warehouse->id;
        ...
        $product_data = [$product_code, $product_name, $product_qty, $product_type, $product_id, $product_list, $qty_list, $product_price, $batch_no, $product_batch_id, $expired_date, $is_embeded,$variantIds];
        return $product_data;
    }
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            $currency_list = DB::table('currencies')->where('code','Euro')->get();
            $numberOfInvoice = Sale::count();
            $custom_fields = CustomField::where('belongs_to', 'sale')->get();
        ...
        if(($category_id != 0) && ($brand_id != 0)){
            $lims_product_list = DB::table('products')
                                ...
                                ->where([
                                    ...
                                    ['products.is_active', true],
                                    ['brand_id', $brand_id]
                                ])->select('products.id','products.name', 'products.code', 'products.image','product_images.src','product_variants.id as varId')->get();
        }
        elseif(($category_id != 0) && ($brand_id == 0)){
            $lims_product_list = DB::table('products')
                                ...
                                ->where([
                                    ...
                                    ['categories.parent_id', $category_id],
                                    ['products.is_active', true]
                                ])->select('products.id', 'products.name', 'products.code', 'products.image', 'products.is_variant','product_images.src','product_variants.id as varId')->get();
        }
        elseif(($category_id == 0) && ($brand_id != 0)){
                $lims_product_list = Product::where([
                        ['brand_id', $brand_id],
        ...
        foreach ($lims_product_list as $product) {
            if($product->is_variant) {
                ...
                foreach ($lims_product_variant_data as $key => $variant) {
                    ...
                    $varId = explode(",", $product->varId);
                    $data['var_id'][$index] = $varId[0];
                    $index++;
                }
            }
            else {
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        $pro_detail =  explode("(",$product_info[0]);
        $pro_id = $pro_detail[0];
        $var_id = $pro_detail[1];
        // dd($var_id);
        ...
        $all_discount = DB::table('discount_plan_customers')
                        ...
                        ->select('discounts.*')
                        ->get();
        $lims_product_data = Product::where([['id', $pro_id],['is_active', true]])->first();
        if($lims_product_data) {
            $lims_product_data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
            ->select('products.*', 'product_variants.id as product_variant_id', 'product_variants.title', 'product_variants.price')
            ->where([
                ['product_variants.product_id', $pro_id],
                ['products.is_active', true]
            ])->first();
    ...
    public function checkDiscount(Request $request)
    {
        ...
        // $lims_product_data = Product::select('id', 'price', 'promotion', 'promotion_price', 'last_date')->where('id',$request->input('product_id'))->with('productVariants')->first();
        ...
        $lims_product_data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
        ->select('products.id','products.promotion','products.promotion_price','products.last_date', 'product_variants.id as product_variant_id', 'product_variants.title', 'product_variants.price')
        ->where([

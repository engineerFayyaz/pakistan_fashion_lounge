...
class ReportController extends Controller
{
    public function availableStock()
    {
        ...
        if($role->hasPermissionTo('product-qty-alert')){
            $lims_product_data = Product::select('name','code', 'image', 'qty', 'alert_quantity')->where('is_active', true)->whereColumn('alert_quantity', '>', 'qty')->get();
                ...
                $lims_product_data2 = DB::table('product_warehouse')
                ->select('products.name','products.code', 'products.id', 'products.image', 'product_warehouse.qty', 'product_warehouse.price', 'products.alert_quantity')
                ->join('products', 'product_warehouse.product_id', '=', 'products.id')
                ->where([
                    ['products.is_active', true]
                ])->get();
            ...
            return view('backend.report.available_stock_report', compact('lims_product_data2','total_qty','total_item'));
        }
        else
    ...
    //         return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    // }
    public function dailySaleNew(Request $request)
    {
        $role = Role::find(Auth::user()->role_id);
        if($role->hasPermissionTo('daily-sale')){
            $biller_id ='';
            ...
            if($request->biller_id){
                ...
                $query->where('biller_id', $biller_id);   
            }
            $sale_data = $query
            ->selectRaw('DATE(created_at) as sale_date, 
                         ...
                         MAX(biller_id) as biller_id, 
                         MAX(currency_id) as currency_id')
            ->groupBy('sale_date')
            ->with(['biller', 'currency'])
            ->get();
        
            $lims_biller_all = biller::where('is_active', true)->get();
            
           
            return view('backend.report.daily_sale_new', compact('biller_id','lims_biller_all','sale_data','date'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function profitLossNew(Request $request)
    {
        $start_date = $request['start_date'];
        $end_date = $request['end_date'];
        $query1 = array(
        ...
        return view('backend.report.profit_loss_new', compact('purchase', 'product_cost', 'product_tax', 'total_purchase', 'sale', 'total_sale', 'return', 'purchase_return', 'total_return', 'total_purchase_return', 'expense', 'payroll', 'total_expense', 'total_payroll', 'payment_recieved', 'payment_recieved_number', 'cash_payment_sale', 'cheque_payment_sale', 'credit_card_payment_sale', 'gift_card_payment_sale', 'paypal_payment_sale', 'deposit_payment_sale', 'payment_sent', 'payment_sent_number', 'cash_payment_purchase', 'cheque_payment_purchase', 'credit_card_payment_purchase', 'warehouse_name', 'warehouse_sale', 'warehouse_purchase', 'warehouse_return', 'warehouse_purchase_return', 'warehouse_expense', 'start_date', 'end_date'));
    }
    ...
    public function saleReportNew(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        ...
        if($data['biller_id']){
            $query->whereDate('biller_id', $biller_id);   
        }
        ...
        $lims_biller_all = biller::where('is_active', true)->get();
        $lims_biller_all = biller::where('is_active', true)->get();
        return view('backend.report.sale_report_new',compact('biller_id','lims_biller_all','sales','start_date', 'end_date'));
    }
    public function saleReportCat(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        ...
        if($request->cat_id){
            ...
            foreach($request->cat_id as $key => $val){
                $cats[]=$key;
            }
            
            $p=Product::whereIn('category_id',$cats)->pluck('id');
        ...
        ->selectRaw('
                     ...
                     MAX(currency_id) as currency_id')
        ->groupBy('item')
        ->with(['biller', 'currency','product','payment'])
        ->get();
       
        $lims_biller_all = biller::where('is_active', true)->get();
        $categories = Category::where('is_active', true)->get();
        return view('backend.report.sale_report_cat',compact('cat_id','categories','biller_id','lims_biller_all','sales','start_date', 'end_date','cats'));
    }
    ...
    public function taxReport(Request $request)
    {
        ...
        $total_sale_amount_query = Product_Sale::query();
        $total_product_tax_query = Product_Sale::query();
        $total_order_tax_query = Product_Sale::query();

        if($data['start_date']){
            $total_sale_amount_query->whereDate('created_at', '>=' , $start_date);
            $total_product_tax_query->whereDate('created_at', '>=' , $start_date);
            $total_order_tax_query->whereDate('created_at', '>=' , $start_date);
        }
        if($data['end_date']){
            $total_sale_amount_query->whereDate('created_at', '<=' , $end_date);   
            $total_product_tax_query->whereDate('created_at', '<=' , $end_date);   
            $total_order_tax_query->whereDate('created_at', '<=' , $end_date);  
        }
        if($data['biller_id']){
            $total_sale_amount_query->where('biller_id', $biller_id);   
            $total_product_tax_query->where('biller_id', $biller_id);   
            $total_order_tax_query->where('biller_id', $biller_id);  
        }

        $total_sale_amount = $total_sale_amount_query->sum('total');
        $total_product_tax = $total_product_tax_query->sum('tax_rate');
        $total_order_tax = $total_order_tax_query->sum('tax');


        return view('backend.report.tax_report',compact('total_order_tax','total_sale_amount','total_product_tax','cat_id','categories','biller_id','lims_biller_all','sales','start_date', 'end_date'));
    }

    public function purchaseTaxReport(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        ...
        ->selectRaw('
                     ...
                     MAX(currency_id) as currency_id')
        ->groupBy('item')
        ->with(['currency','product'])
        ->get();
       
        $lims_biller_all = biller::where('is_active', true)->get();
        $categories = Category::where('is_active', true)->get();

        $total_sale_amount_query = Purchase::query();
        $total_product_tax_query = Purchase::query();
        $total_order_tax_query = Purchase::query();
        ...
        $total_sale_amount = $total_sale_amount_query->sum('total_cost');
        $total_product_tax = $total_product_tax_query->sum('order_tax_rate');
        $total_order_tax = $total_order_tax_query->sum('order_tax');


        return view('backend.report.purchase_tax_report',compact('total_order_tax','total_sale_amount','total_product_tax','cat_id','categories','lims_biller_all','sales','start_date', 'end_date'));
    }
    ...
    public function customerReportNew(Request $request)
    {
        $customer_id = $request->input('customer_id');
        if($request->input('start_date')) {
            $start_date = $request->input('start_date');
        ...
        $lims_customer_list = Customer::where('is_active', true)->get();
        $credit['limit'] = Customer::where('id', $customer_id)->sum('credit_limit');
        $credit['balance'] = Customer::where('id', $customer_id)->sum('credit_balance');
        return view('backend.report.customer_report_new',compact('credit','start_date', 'end_date', 'customer_id', 'lims_customer_list'));
    }
    ...
    public function customerCreditReportByDate(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        ...
        $c_ids=$q->pluck('customer_id');
    
        $credit['limit'] = Customer::whereIn('id', $c_ids)->sum('credit_limit');
        $credit['balance'] = Customer::whereIn('id', $c_ids)->sum('credit_balance');
        return view('backend.report.credit_report', compact('credit','lims_sale_data', 'start_date', 'end_date'));
    }
    public function customerCreditReportByDateAll(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        ...
        return view('backend.report.credit_report_all', compact('credit','lims_sale_data', 'start_date', 'end_date'));
    }
    public static function customerCreditData()
    {
        $credit['limit'] = Customer::sum('credit_limit');
        $credit['balance'] = Customer::sum('credit_balance');
        return $credit;
    }
    ...
    public function customerDueReportData(Request $request)
    {
        ...
        $q = DB::table('sales')
            ...
            $order = 'sales.'.$columns[$request->input('order.0.column')];
            $dir = $request->input('order.0.dir');
            $q = $q->select('sales.id', 'sales.reference_no', 'sales.grand_total', 'sales.created_at', 'sales.paid_amount', 'customers.name as customer_name', 'customers.credit_limit as credit_limit', 'customers.credit_balance as credit_balance', 'customers.phone_number as customer_phone_number')
                ->offset($start)
                ->limit($limit)
                ->orderBy($order, $dir);
            ...
            if(!empty($sales))
            {
                foreach ($sales as $key => $sale)
                {
                    ...
                    $nestedData['credit_limit'] = $sale->credit_limit;
                    $nestedData['credit_balance'] = $sale->credit_balance;
                    $nestedData['grand_total'] = number_format($sale->grand_total, cache()->get('general_setting')->decimal);

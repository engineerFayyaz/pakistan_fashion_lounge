    ...
    public function dashboard()
    {
        ...
        else 
        {
            $product_sale_data = Product_Sale::join('sales', 'product_sales.sale_id', '=', 'sales.id')
                                ...
                                ->get();
            $product_cost = $this->calculateAverageCOGS($product_sale_data);
            $sales = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->count();
            $customers = Customer::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->count();
            $inventory = Product::where('is_active',1)->count();
        ...
        return view('backend.index', compact('sales','customers','inventory','revenue', 'purchase', 'expense', 'return', 'purchase_return', 'profit', 'payment_recieved', 'payment_sent', 'month', 'yearly_sale_amount', 'yearly_purchase_amount', 'alertBugEnable','alertVersionUpgradeEnable'));
    }

    ...
    <div id="loader"></div>
      ...
      <nav class="side-navbar">
        <span class="brand-big">
            @if(isset($general_setting->site_logo) && $general_setting->site_logo != '')
              <a href="{{url('/')}}"><img src="{{asset('assets/images/'.$general_setting->site_logo)}}" width="100"></a>
            @else
      ...
      <div id="customer-modal2" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade text-left">
        <div role="document" class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                    {!! Form::open(['route' => 'report.customerNew', 'method' => 'post']) !!}
                    <?php
                      $lims_customer_list = DB::table('customers')->where('is_active', true)->get();
                    ?>
                      <div class="form-group">
